...
function reduce(state: XRayState, msg: ServerMessage): XRayState {
  switch (msg.type) {
    ...
    case "step": {
      // Replace-or-append by step index, then keep ordered — robust to any
      // arrival order (the server streams them in order regardless).
      const steps = [
...
export interface UseXRay extends XRayState {
  run: (prompt: string, thinking?: boolean) => void;
}
...
export function useXRay(): UseXRay {
  ...
  const run = useCallback((prompt: string, thinking: boolean = true) => {
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({ prompt: trimmed, thinking }));
